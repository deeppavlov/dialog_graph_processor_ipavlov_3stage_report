...
pathlib.Path("test_dir/japanese_multiwoz_dir").mkdir(parents=True, exist_ok=True)

DATA_PATH = "data/japanese_multiwoz.json"
EMBEDDINGS_FILE = "test_dir/japanese_multiwoz_dir/embeddings.npy"
language = "en"
num_speakers = 2
...
intent_predictor.dump_dialog_graph("./test_dir/japanese_multiwoz_dir/dialog_graph")
...
intent_predictor.init_message_passing_model("./test_dir/japanese_multiwoz_dir")

graph_model_end = time.time()

graph_model_metrics_start = time.time()

intent_predictor.get_message_passing_metrics("./test_dir/japanese_multiwoz_dir")
...
print(f"Получены метрики. Время расчета метрик графовой модели {graph_model_end - graph_model_start}s")